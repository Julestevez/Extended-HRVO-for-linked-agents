...
def visualize_traj_dynamic(ws_model, X, U, goal, triads, time = None, name=None):
    ...
    # Plot lines between triads
    for triad in triads:
        for i in range(3):
            ax.plot([X[triad[i]][0], X[triad[(i+1)%3]][0]], 
                    [X[triad[i]][1], X[triad[(i+1)%3]][1]], 
                    'k-', linewidth=1.0)
    ...
    # ---set axes ---
    ax.set_aspect('equal')
    ax.set_xlim(-1.0, 11.0)  # Adjusted x-axis limit
    ax.set_ylim(-1.0, 6.0)
    ax.set_xlabel(r'$x (m)$')
    ax.set_ylabel(r'$y (m)$')
    ax.grid(True)
