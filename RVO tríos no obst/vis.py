...
def visualize_traj_dynamic(ws_model, X, U, goal, groups, time = None, name=None):
    ...
    # Plot lines connecting circles within each group
    for group in groups:
        # Draw line from first to second circle
        ax.plot([X[group[0]][0], X[group[1]][0]], [X[group[0]][1], X[group[1]][1]], 'k-', linewidth=1.0)
        # Draw line from second to third circle
        ax.plot([X[group[1]][0], X[group[2]][0]], [X[group[1]][1], X[group[2]][1]], 'k-', linewidth=1.0)
    ...
    # ---set axes ---
    ax.set_aspect('equal')
    ax.set_xlim(-1.0, 19.0)  # Adjusted x-axis limit
    ax.set_ylim(-1.0, 6.0)
    ax.set_xlabel(r'$x (m)$')
    ax.set_ylabel(r'$y (m)$')
    ax.grid(True)
