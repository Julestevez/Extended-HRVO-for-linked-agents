...
def RVO_update(X, V_des, V_current, ws_model, triads):
    ...
    for i in range(len(X)):
        ...
        # Find this agent's triad
        triad_indices = next((triad for triad in triads if i in triad), None)
        ...
        for j in range(len(X)):
            if i!=j:
                ...
                # If j is in the same triad as i, use a larger radius
                if triad_indices and j in triad_indices:
        ...
        # Add collision avoidance for links within triads
        for triad in triads:
            if i not in triad:
                for k in range(3):
                    p1, p2 = X[triad[k]], X[triad[(k+1)%3]]
