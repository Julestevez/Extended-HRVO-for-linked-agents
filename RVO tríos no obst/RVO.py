from math import ceil, floor, sqrt, cos, sin, tan, atan2, asin, pi as PI
import copy
import numpy
...
def RVO_update(X, V_des, V_current, ws_model, groups):
    ...
    for i in range(len(X)):
        ...
        # Find this agent's group
        agent_group = next((group for group in groups if i in group), None)
        ...
        for j in range(len(X)):
            if i!=j:
                ...
                # If j is in the same group as i, use a larger radius
                if agent_group and j in agent_group:
        ...
        # Add collision avoidance for lines connecting circles in other groups
        for group in groups:
            if i not in group:
                # Create two line segments for each group
                line1 = (X[group[0]], X[group[1]])
                line2 = (X[group[1]], X[group[2]])
                
                for line in [line1, line2]:
                    closest_point = closest_point_on_segment(pA, line[0], line[1])
                    dist_BA = distance(pA, closest_point)
                    
                    # Increase the effective radius of the line
                    line_radius = 2 * ROB_RAD
                    
                    if dist_BA < line_radius:
                        dist_BA = line_radius
                    
                    theta_BA = atan2(closest_point[1]-pA[1], closest_point[0]-pA[0])
                    theta_BAort = asin(line_radius/dist_BA)
                    theta_ort_left = theta_BA+theta_BAort
                    bound_left = [cos(theta_ort_left), sin(theta_ort_left)]
                    theta_ort_right = theta_BA-theta_BAort
                    bound_right = [cos(theta_ort_right), sin(theta_ort_right)]
                    
                    RVO_BA = [closest_point, bound_left, bound_right, dist_BA, line_radius]
                    RVO_BA_all.append(RVO_BA)
