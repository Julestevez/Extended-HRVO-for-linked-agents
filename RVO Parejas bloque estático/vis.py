...
def visualize_traj_dynamic(ws_model, X, U, goal, pairs, time = None, name=None):
    ...
    for pair in pairs:
        ax.plot([X[pair[0]][0], X[pair[1]][0]], [X[pair[0]][1], X[pair[1]][1]], 'k-', linewidth=1.0)
    ...
    # ---set axes ---
    ax.set_aspect('equal')
    ax.set_xlim(-1.0, 9.0)  # Adjusted x-axis limit
    ax.set_ylim(-1.0, 6.0)
    ax.set_xlabel(r'$x (m)$')
    ax.set_ylabel(r'$y (m)$')
    ax.grid(True)
