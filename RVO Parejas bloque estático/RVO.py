...
def RVO_update(X, V_des, V_current, ws_model, pairs):
    ...
    for i in range(len(X)):
        ...
        # Add collision avoidance for static obstacles
        for obstacle in ws_model['static_obstacles']:
            closest_point = closest_point_on_rectangle(pA, obstacle)
            dist_BA = distance(pA, closest_point)
            if dist_BA < ROB_RAD:
                dist_BA = ROB_RAD
            theta_BA = atan2(closest_point[1]-pA[1], closest_point[0]-pA[0])
...
def closest_point_on_rectangle(p, rect):
    """
    Find the closest point on a rectangle to point p
